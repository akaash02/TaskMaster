...
const EventScreen = ({ navigation, route }) => {
  ...
  if (!theme) {
    return (
      <View style={styles.container}>
        <Text testID="themeErrorText" style={styles.errorText}>Theme context is not available.</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <Text testID="eventScreenTitle" h4 style={[styles.title, { color: theme.colors.text }]}>Create Event</Text>
      <TextInput
      ...
      <TouchableOpacity onPress={() => setStartTimePickerVisibility(true)}>
        <Text testID="startTimeText" style={[styles.input, { color: theme.colors.text }]}>
          {startTime ? startTime.toLocaleString() : 'Start Time'}
        </Text>
      </TouchableOpacity>
      <TouchableOpacity onPress={() => setEndTimePickerVisibility(true)}>
        <Text testID="endTimeText" style={[styles.input, { color: theme.colors.text }]}>
      ...
      />
      <TouchableOpacity onPress={handleSave} style={[styles.button, { backgroundColor: theme.colors.card }]} testID="saveEventButton">
        <Text style={[styles.buttonText, { color: theme.colors.text }]}>Save Event</Text>
      </TouchableOpacity>
      <TouchableOpacity onPress={() => navigation.navigate('Home')} style={[styles.button, { backgroundColor: theme.colors.card }]} testID="homeButton">
        <Text style={[styles.buttonText, { color: theme.colors.text }]}>Home</Text>
      </TouchableOpacity>
      <Text testID="offlineText" style={[styles.offlineText, { color: theme.colors.text }]}>If offline just click save once and press home</Text>
    </ScrollView>
  );
};
